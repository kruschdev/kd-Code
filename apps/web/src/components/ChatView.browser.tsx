...
import { scopedThreadKey, scopeThreadRef } from "@krusch/client-runtime";
...
import { DEFAULT_CLIENT_SETTINGS } from "@krusch/contracts/settings";
  ...
  it("keeps the wide desktop follow-up layout expanded when the footer still fits", async () => {
    const mounted = await mountChatView({
      ...
      snapshot: createSnapshotWithPlanFollowUpPrompt({
        ...
        planMarkdown:
          "# Imaginary Long-Range Plan: Krusch DBOS Adaptive Orchestration and Safe-Delay Execution Initiative",
      }),
    });
