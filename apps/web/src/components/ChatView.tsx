...
import { resolveOptimalRoute } from "../services/routerService";
...
export default function ChatView(props: ChatViewProps) {
  ...
  const onSend = async (e?: { preventDefault: () => void }) => {
    ...
    // 1. Context Anchoring: Retrieve latest compiled project state from Postgres plane
    try {
      const projectState = await fetchProjectState(activeProject?.name || "krusch-ide");
      if (projectState) {
        messageTextForSend += formatContextEnvelope(projectState);
      }
    } catch (e) {
      console.warn("[KD Code Context] Failed to fetch project state:", e);
    }

    // 2. Intelligent Multi-Model Routing: evaluate if Auto router is selected
    let effectiveProvider = ctxSelectedProvider;
    let effectiveModel = ctxSelectedModel;
    let effectiveModelSelection = ctxSelectedModelSelection;

    if (ctxSelectedModel === "auto:cascade" || ctxSelectedModel === "auto") {
      try {
        const route = await resolveOptimalRoute(promptForSend);
        effectiveProvider = route.recommendedProvider;
        effectiveModel = route.recommendedModel;
        effectiveModelSelection = createModelSelection(
          effectiveProvider,
          effectiveModel,
        ...
        toastManager.add(
          stackedThreadToast({
            type: "info",
            title: `KD Code Router (${route.stage === "L1_FAST_PATH" ? "L1 Fast-Path" : "L2 Neural"})`,
            description: `Routed prompt to ${route.recommendedModel} (${route.role})`,
          }),
        );
      } catch (err) {
        console.warn("[KD Code Router] Dynamic routing error:", err);
      }
    }
    ...
    await (async () => {
      ...
      const threadCreateModelSelection = createModelSelection(
        ...
        effectiveModelSelection.options,
      );
      ...
      if (isServerThread) {
        await persistThreadSettingsForNextTurn({
          threadId: threadIdForSend,
          createdAt: messageCreatedAt,
          ...(effectiveModel ? { modelSelection: effectiveModelSelection } : {}),
          runtimeMode,
          interactionMode,
        });
      }
