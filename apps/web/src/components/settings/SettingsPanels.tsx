...
const PROVIDER_SETTINGS: readonly InstallProviderSettings[] = [
  ...
  },
  {
    ...
    serverUrlDescription: "Leave blank to let Krusch DBOS spawn the server when needed",
  ...
  },
  {
    ...
    apiKeyDescription: "If provided, bypasses the CLI and connects directly to the Gemini API.",
  },
] as const;
...
export function GeneralSettingsPanel() {
  ...
  const [openProviderDetails, setOpenProviderDetails] = useState<Record<ProviderKind, boolean>>({
    ...
    gemini: Boolean(settings.providers.gemini.customModels.length > 0),
  });
  ...
  const providerCards = visibleProviderSettings.map((providerSettings) => {
    ...
    return {
      ...
      apiKeyValue: "apiKey" in providerConfig ? providerConfig.apiKey : "",
  ...
  return (
    <SettingsPageContainer>
      <SettingsSection title="General">
        <SettingsRow
          title="Theme"
          description="Choose how Krusch DBOS looks across the app."
          resetAction={
            theme !== "system" ? (
              ...
              >
                <CollapsibleContent>
                  <div className="space-y-0">
                    ...
                    {providerCard.apiKeyPlaceholder ? (
                      <div className="border-t border-border/60 px-4 py-3 sm:px-5">
                        <label
                          htmlFor={`provider-install-${providerCard.provider}-api-key`}
                          className="block"
                        >
                          <span className="text-xs font-medium text-foreground">
                          ...
                          <Input
                            id={`provider-install-${providerCard.provider}-api-key`}
                            className="mt-1.5"
                            type="password"
                            autoComplete="off"
                            value={providerCard.apiKeyValue}
                            onChange={(event) =>
                              updateSettings({
                                providers: {
                                  ...settings.providers,
                            ...
                            spellCheck={false}
                          />
                          {providerCard.apiKeyDescription ? (
                            <span className="mt-1 block text-xs text-muted-foreground">
                              {providerCard.apiKeyDescription}
                            </span>
                          ) : null}
      ...
      <SettingsSection title="Semantic Recall">
        <SettingsRow
          title="Enable Semantic Recall"
          description="Allows the AI to remember historical decisions and actions by searching past conversations using vector embeddings."
          control={
            <Switch
              checked={settings.embeddingModelSelection.provider !== "none"}
              onCheckedChange={(checked) => {
                updateSettings({
                  embeddingModelSelection: {
                    ...settings.embeddingModelSelection,
                    provider: checked ? "ollama" : "none",
                  },
                });
              }}
            />
        ...
        />
        {settings.embeddingModelSelection.provider !== "none" && (
          <>
            <SettingsRow
              title="Embedding Provider"
              description="Select the service used to generate vector embeddings for your messages."
              control={
                <Select
                  value={settings.embeddingModelSelection.provider}
                  onValueChange={(value) => {
                    updateSettings({
                      embeddingModelSelection: {
                        ...settings.embeddingModelSelection,
                        provider: value as EmbeddingProviderKind,
                        model:
                          value === "ollama"
                            ? "nomic-embed-text"
                            : value === "gemini"
            ...
            <SettingsRow
              ...
              description="The specific model used for embedding generation."
              control={
                <Input
                  className="w-full sm:w-64"
                  value={settings.embeddingModelSelection.model}
                  onChange={(e) =>
                    updateSettings({
                      embeddingModelSelection: {
                        ...settings.embeddingModelSelection,
            ...
            />
            {settings.embeddingModelSelection.provider === "ollama" && (
              <SettingsRow
                title="Ollama API Endpoint"
                description="The URL of your local Ollama instance."
                control={
                  <Input
                    className="w-full sm:w-64"
                    value={settings.embeddingModelSelection.apiEndpoint}
                    onChange={(e) =>
                      updateSettings({
                        embeddingModelSelection: {
                          ...settings.embeddingModelSelection,
            ...
            {(settings.embeddingModelSelection.provider === "gemini" ||
              settings.embeddingModelSelection.provider === "openai") && (
              <SettingsRow
                title="API Key"
                description={`API Key for ${settings.embeddingModelSelection.provider === "gemini" ? "Google Gemini" : "OpenAI"}.`}
                control={
                  <Input
                    className="w-full sm:w-64"
                    type="password"
                    value={settings.embeddingModelSelection.apiKey}
                    onChange={(e) =>
                      updateSettings({
                        embeddingModelSelection: {
                          ...settings.embeddingModelSelection,
