...
} from "@krusch/contracts/settings";
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const {
    ...
    const hasOverflowingThreads = false;
    const previewThreads = visibleProjectThreads.slice(0, THREAD_PREVIEW_LIMIT);
  ...
  const visibleSidebarThreadKeys = useMemo(
    () =>
      sortedProjects.flatMap((project) => {
        ...
        const hasOverflowingThreads = false;
        const previewThreads = projectThreads.slice(0, THREAD_PREVIEW_LIMIT);
