...
import { createModelSelection, normalizeModelSlug } from "@krusch/shared/model";
...
import { UnifiedSettings } from "@krusch/contracts/settings";
...
function normalizeProviderModelOptions(
  ...
  const geminiThinking =
    geminiCandidate?.thinking === true
      ? true
      : geminiCandidate?.thinking === false
        ? false
        : undefined;
  const geminiFastMode =
    geminiCandidate?.fastMode === true
      ...
      : geminiCandidate?.fastMode === false
        ? false
        : undefined;
  const geminiContextWindow =
    typeof geminiCandidate?.contextWindow === "string" && geminiCandidate.contextWindow.length > 0
      ? geminiCandidate.contextWindow
      : undefined;
  const gemini =
    geminiThinking !== undefined ||
    geminiFastMode !== undefined ||
    geminiContextWindow !== undefined
      ? {
          ...(geminiThinking !== undefined ? { thinking: geminiThinking } : {}),
          ...(geminiFastMode !== undefined ? { fastMode: geminiFastMode } : {}),
          ...(geminiContextWindow !== undefined ? { contextWindow: geminiContextWindow } : {}),
        }
      : undefined;
