...
process.on("uncaughtException", (error: NodeJS.ErrnoException) => {
  if (error.code === "EIO") {
    // Ignore EIO errors from broken stdout when detached
    return;
  }
  console.error("Uncaught exception:", error);
});
process.on("unhandledRejection", (reason: unknown) => {
  if (reason instanceof Error && (reason as NodeJS.ErrnoException).code === "EIO") {
    return;
  }
...
import type { ContextMenuItem } from "@krusch/contracts";
import { RotatingFileSink } from "@krusch/shared/logging";
import { parsePersistedServerObservabilitySettings } from "@krusch/shared/serverSettings";
...
const LEGACY_USER_DATA_DIR_NAME = isDevelopment ? "Krusch DBOS (Dev)" : "Krusch DBOS (Alpha)";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  const envFileVars = parseEnvFile(Path.join(BASE_DIR, ".env"));
  const env = { ...envFileVars, ...process.env };
  console.log("DEBUG_DATABASE_URL:", env.DATABASE_URL);
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("Krusch DBOS failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `Krusch DBOS ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/Krusch DBOS (Alpha)` on Linux). This is
...
function startBackend(): void {
  ...
  const child = ChildProcess.spawn(
    isDevelopment ? "bun" : process.execPath,
    ...
    void waitForBackendWindowReady(backendHttpUrl)
      ...
      .catch((error) => {
        ...
        try {
          ...
          // Ignore EIO errors from broken stdout
        }
      });
    return;
  }
